...
| `glean/notify.py` | 推送四通道 `file`/Web NEW/`desktop`/`webhook`（generic·feishu·wecom），全 fail-soft | ✅ |
| `glean/web/main.py` | FastAPI 应用工厂、静态挂载；`watch_new_count` 模板全局（导航未读角标） | ✅ |
...
| `glean/templates/**` | Jinja2 模板（base / digest / profile / archive / **watch** + 3 个 partial） | ✅ |
| `glean_static/**` | `css/app.css`、`js/app.js`（Alpine 键盘流） | ✅ |
| `watchlist.md` | **监控名单真相源**：姓名/主页/DBLP/S2/tags/enabled，两节（监控中/已暂停） | ✅ |
...
覆盖 4 页面 + 6 个 API（含 `/api/ping`）+ 1 个 HTMX 片段 + 8 个 `/api/watch/*` 端点
（4 只读 + add/toggle/delete/ack 四个写操作，均隔离到 `tmp_path` 不碰真实文件））、
`tests/test_serve.py`（`probe` / `ensure` 单元 + 端到端环回服务存活测试，含环境代理绕过回归）、
`tests/test_watch.py`（名单增删启停、解析优先级、指纹、基线/差异、事件、digest 幂等）、
`tests/test_notify.py`（文件通道去重/ack、桌面通道开关、webhook 三种载荷与失败降级）、
`tests/test_cli.py`（子进程验证 CLI 七个子命令与包装器，含新增的 `watch add/remove/list/run` 八个子命令组）。共 **73 个测试，全部通过**（`pytest -q` → `73 passed`）。
